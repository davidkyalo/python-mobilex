...
from mobilex.utils.local import PromiseProxy, Proxy, maybe_evaluate, try_import
...
class test_Proxy:
    ...
    def test_std_class_attributes(self):
        ...
        assert Proxy.__module__ == 'mobilex.utils.local'
    ...
    def test_name(self):
        ...
        x = Proxy(lambda: real, __name__='xyz')
        assert x.__name__ == 'xyz'

        y = Proxy(lambda: real)
        assert y.__name__ == 'real'
        assert y.__qualname__ == real.__qualname__
